...
class AESECB : ComponentActivity() {
...
fun Greeting(name: String, modifier: Modifier = Modifier) {
    Surface(color = Color.LightGray) {
        ...
        val zHardwareByte = zHardware.toByteArray()
        ...
        val cipher =
            Cipher.getInstance("AES/ECB/PKCS5Padding") //getting the instance of AES - i.e. setting it up
        cipher.init(
            Cipher.ENCRYPT_MODE,
            keyspec
        ) //initializing the cipher and the specific mod needed.
        ...
        val dataEncryptedAbove =
            Base64.getDecoder().decode(cipherTextString) //Decrypting the string back to binary.
        cipher.init(Cipher.DECRYPT_MODE, keyspec) //setting up the decrypt instance of AES
        val returnText = cipher.doFinal(dataEncryptedAbove) //doing the decryption
        val returnTextString =
            String(returnText) //turning the final output from a Byte Array into a string that can be outputted.
