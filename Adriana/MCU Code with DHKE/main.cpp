    ...
    case ESP_SPP_DATA_IND_EVT:
        ...
        if (param->data_ind.len < 128) {
            ...
            if (process.compare("DKE") == 0) {
                ...
                printf("Entered DKE2\n");
                //DKE2 is when ciphertext comes in 
                //TO DO: decrypt ciphertext (sk+z-hardware) to get z-hardware
                ...
                //Open NVS
                /*TO DO: split data into two variables*/

                string* handle_name; //vehicle plain text
                string* z_hardware; //z-hardware profile 
                /*nvs_handle_t handle_name;
                err = nvs_open("storage", NVS_READWRITE, &handle_name);
                if (err != ESP_OK) {
                    printf("Error (%s) opening NVS handle!\n", esp_err_to_name(err));
                } else {
                    printf("Done\n");
                    ...
                    err = nvs_set_i32(handle_name, "z-hardware profile", z_hardware);
                    printf((err != ESP_OK) ? "Failed!\n" : "Done\n");

                    //commit written value
                    err = nvs_commit(handle_name);
                    printf((err != ESP_OK) ? "Failed!\n" : "Done\n");

                    //close
                    nvs_close(handle_name);
                }*/
            }
            else if (process == "COM") {
            ...
            //TO DO: clear all variables when done with computation
        }
#else
