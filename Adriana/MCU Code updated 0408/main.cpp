    ...
    case ESP_SPP_DATA_IND_EVT:
        ...
        if (param->data_ind.len < 128) {
            ...
            if (process == "DKE" /*process.compare("DKE") == 0)*/) {
                ...
                string handle_name; //car name from data_receieved
                string z_hardware; //variable to eventually store z-hardware when read from NVS
                string unix_time; // separated from data_received
                string command; //separated from data_receieved
                string received_hash; //separated from data_receieved

                // Separate all variables from data_received
                // data_received is hash | command | unix time | handle name
                int pos = data_received.find("|"); // find first delimiter point
                received_hash = data_received.substr(0, pos); // store hash
                data_received = data_received.substr(pos + 1); // erase hash from data_received
                printf("received_hash: %s\n", received_hash.c_str());
                printf("new data_received string after removing received_hash: %s\n", data_received.c_str());

                // data_received: command + unix time + handle name
                pos = data_received.find("|"); // find second delimiter
                command = data_received.substr(0, pos); // store command
                data_received = data_received.substr(pos + 1); // erase command from data_received
                printf("command: %s\n", command.c_str());
                printf("new data_received string after removing command: %s\n", data_received.c_str());

                // data_received: unix time + handle name
                pos = data_received.find("|"); // find third delimiter
                unix_time = data_received.substr(0, pos); // store unix time
                data_received = data_received.substr(pos + 1); // erase unix time from data_received
                printf("unix_time: %s\n", unix_time.c_str());
                printf("new data_received string after removing unix_time: %s\n", data_received.c_str());

                // data_received: handle name
                handle_name = data_received; // store handle name
                ...
                //Code to read z-hardware from NVS storage
                //Initialize NVS
                esp_err_t err = nvs_flash_init();
                if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
                    // NVS partition was truncated and needs to be erased
                ...
                if (err != ESP_OK) {
                    ...
                    //read z-hardware from nvs
                    size_t required_size;
                    nvs_get_str(nvs_handle, "z hardware", NULL, &required_size);
                    char* zh = (char*)malloc(required_size);
                    err = nvs_get_str(nvs_handle, "z hardware", zh, &required_size);
                    switch (err) {
                        case ESP_OK:
                            printf("Done\n");
                            printf("z_hardware: %s\n", zh);
                    ...
                    z_hardware = zh; //assign z-hardware string from char*
                ...
                //SHA
                //assign byte
                string msgStart; //starting message for SHA
                msgStart.assign(z_hardware);
                msgStart.append(command);
                msgStart.append(unix_time);
                printf("string for SHA: %s\n", msgStart.c_str());
                //msgStart: z_hardware + command + unix_time
                string msgDigest; //ending message for SHA
                string DDS;
                CryptoPP::HexEncoder encoder(new CryptoPP::StringSink(DDS));
                ...
                hashAlgorithm.Final((CryptoPP::byte*)&msgDigest[0]); //does the SHA hash algorithm using a pointer
                CryptoPP::StringSource(
                    msgDigest,
                    true,
                    new CryptoPP::Redirector(encoder));
                printf("ending string DDS: %s\n", DDS.c_str());
                printf("received_hash from app: %s\n", received_hash.c_str());

                //LED turn on/off code
                if(DDS == received_hash){

                    if(command == "Mode00"){ //Red LED OFF
                        gpio_set_level(ledR, 0);
                        break;
                    }
                    else if (command == "Mode01"){ //Red LED ON
                        gpio_set_level(ledR, 1);
                        break;
                    }
                    else if (command == "Mode10"){ //Green LED OFF
                       gpio_set_level(ledG, 0);
                        break; 
                    }
                    else if (command == "Mode11"){ //Green LED ON
                        gpio_set_level(ledG, 1);
                        break;
                    }
                    else { //default Green and Red LED both OFF
                        gpio_set_level(ledG, 0);
                        gpio_set_level(ledR, 0); 
                        break;
                    }
